# Hi there 👋, I'm Balaji

### AI/ML Engineer | LLMOps | RAG Systems | Full-Stack AI Applications

<img align="right" alt="coding" width="350" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif">

- 🎓 B.Tech CSE Student at RK Valley
- 🤖 Passionate about AI/ML, LLMOps, and Generative AI
- 🧠 Building RAG pipelines, AI agents, and real-time AI systems
- ⚡ Skilled in FastAPI, LangChain, LangGraph, Docker, and HuggingFace
- 🚀 Focused on deployable AI products and scalable backend systems

---

## 🚀 Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)

---

### AI / ML
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/ScikitLearn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black)

---

### Frameworks & Tools
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)

---

# 📌 Featured Projects

## 🔐 Rakshana AI – Real-Time Scam Detection App
- Multilingual scam detection mobile application
- OCR + Transformer-based classification
- Supports English, Telugu, and Hindi
- Real-time threat detection pipeline

### Stack
`React Native` `FastAPI` `EasyOCR` `DistilBERT` `BERT` `Supabase`

---

## 🧠 AI-Powered Live Meeting Summarizer
- Real-time speech-to-text transcription
- AI-generated meeting summaries
- Automated PDF report generation
- Email delivery integration

### Stack
`Python` `Whisper` `Transformers` `Streamlit`

---

## 🛡️ End-to-End Network Security MLOps Pipeline
- Full ML pipeline for threat detection
- MLflow experiment tracking
- Dockerized deployment
- Real-time REST API inference

### Stack
`Scikit-learn` `MLflow` `Flask` `Docker` `MongoDB`

---

## 📄 AI Resume Generator
- ATS-friendly resume generator
- Local LLM integration
- OCR-based resume parsing
- Multi-template PDF generation

### Stack
`Python` `Supabase` `OCR` `LaTeX`

---

# 🏆 Achievements

- 🥉 3rd Prize – Aadhya Hackathon 2026
- 🚀 Built a multilingual AI scam detection system
- 💡 Interested in scalable AI systems and AI agents

---

# 📊 GitHub Stats

![](https://github-readme-stats.vercel.app/api?username=Balaji-731&show_icons=true&theme=tokyonight)

![](https://github-readme-stats.vercel.app/api/top-langs/?username=Balaji-731&layout=compact&theme=tokyonight)

![](https://github-readme-streak-stats.herokuapp.com/?user=Balaji-731&theme=tokyonight)

---

# 🌐 Connect With Me

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Balaji-731)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](YOUR_LINKEDIN_LINK)

📧 poralabalaji@gmail.com

---

# ✨ Quote

> "Building AI systems that solve real-world problems."
